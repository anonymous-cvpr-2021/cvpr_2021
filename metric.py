...
def compute_dice_score(pred, labels, device):
	'''pred: tensor of [B,C,X,Y,Z]
	   labels: tensor of [B,C,X,Y,Z]'''
	dsc = []
	B, C, X, Y, Z = pred.size()
	_pred = pred.argmax(dim=1).to(device)
	pred = torch.zeros(pred.shape).to(device)

	for i in range(C):
		__pred = torch.zeros(_pred.shape).to(device)
		__pred[_pred==i] = 1
		pred[:,i,:,:,:] = __pred
